<script>
    $(document).ready(function () {
        ...
        var table1 = $('#daftar_data_1').DataTable({
            ...
            "serverSide": true,
            "ordering": false,
            "ajax": "{{ route('Owner Tabel Penjualan Item', $data) }}",
            lengthMenu: [
                [5, 10, 25, 100, -1],
                [5, 10, 25, 100, "All"]
            ],
            pageLength: 10,
            dom: '<"columns row"<"column col-sm-6"l><"column col-sm-6 text-right"{{ (Auth::user()->can("Mengubah Penjualan") ? "B" : "") }}>>,' +
            ...
            buttons: [{
                ...
                action: function (e, dt, node, config) {
                    document.location.href = "{{ route('Owner Penjualan Item Create', $data) }}";
                },
        ...
        var table2 = $('#daftar_data_2').DataTable({
            ...
            "serverSide": true,
            "ordering": false,
            "ajax": "{{ route('Owner Tabel Penjualan Pembayaran', $data) }}",
            lengthMenu: [
                [5, 10, 25, 100, -1],
                [5, 10, 25, 100, "All"]
            ],
            ...
            buttons: [{
                ...
                text: 'Tambah Data',
                action: function (e, dt, node, config) {
                    document.location.href = "{{ route('Owner Penjualan Pembayaran Create', $data) }}";
                },
                init: function (api, node, config) {
                    $(node).removeClass('btn-default')
                }
