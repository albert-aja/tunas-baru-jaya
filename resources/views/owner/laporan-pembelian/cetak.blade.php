...
<html>
	...
	<body>
        ...
        <table style="width: 50%; margin-bottom: 20px; margin-top: 20px; font-size: 8pt; font-weight: light; background-color: #eeeeee;">
            <tr>
                <td style="width: 30%;">Waktu Transaksi</td>
            ...
            <tr>
                ...
                <td style="text-align: left;">{{ ((!is_null($filter['total_pembelian_minimal']) and !is_null($filter['total_pembelian_maksimal'])) ? GeneralHelp::get_currency($filter['total_pembelian_minimal']).' - '.GeneralHelp::get_currency($filter['total_pembelian_maksimal']) : '') }}</td>
            </tr>
            <tr>
        ...
        <table style="width: 100%; margin-top: 20px; margin-bottom: 20px; font-size: 8pt;" class="table">
            <thead>
                <tr style="background-color: #aaaaaa;">
                    <th colspan="6">Laporan Pembelian</th>
                </tr>
                <tr style="background-color: #aaaaaa;">
                    <th>No. Faktur</th>
            ...
            @foreach ($data as $item)
                <tr>
                    ...
                    <td style="text-align: left; border-top: 1px solid black !important;">{{ GeneralHelp::get_currency($item->get_total_harga() - $item->get_total_pembayaran()) }}</td>
                ...
                @foreach ($item->get_item as $item)
                    <tr style="font-size: 6.5pt; background-color: #eeeeee; font-style: italic;">
                        ...
                        <td style="text-align: right;">{{ GeneralHelp::get_currency($item->harga_beli) }}</td>
                        <td style="text-align: right;">{{ GeneralHelp::get_currency($item->kuantitas * $item->harga_beli) }}</td>
